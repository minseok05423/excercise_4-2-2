...
class Scoreboard():
    ...
    def set_score_loc(self):
        n = self.n
        bg_line_height = self.bg_height / (2 * n - 1)
        for score_name, score in self.stat.scores.items():
            self.line_num += 1
            line_loc = self.bg.top + bg_line_height + (self.line_num - 1) * bg_line_height
    ...
    def prep_score(self, score_name, score, line_loc):
        score_line = score_name + " = " + str(score)
        self.score_image = self.font.render(score_line, True, self.font_color, self.bg_color)
        self.score_image_rect = self.score_image.get_rect()
        self.score_image_rect.centerx = self.bg.centerx
        self.score_image_rect.centery = line_loc
        return self.score_image, self.score_image_rect
